...
interface RouteHeaderCardProps {
  title: string;
  subtitle?: string;
  /** Pass asset-picker props to show the picker button in the header */
  assetPicker?: AssetPickerProps;
}

export function RouteHeaderCard({ title, subtitle, assetPicker }: RouteHeaderCardProps) {
  ...
  const assets = assetPicker?.assets ?? [];
  const selectedAssetId = assetPicker?.selectedAssetId ?? null;
  const onUploadBundle = assetPicker?.onUploadBundle;
  const onPickAsset = assetPicker?.onPickAsset;
  const onLoadFromUrl = assetPicker?.onLoadFromUrl;
  const isLoadingSelected = assetPicker?.isLoadingSelected ?? false;
    ...
    () =>
      pendingFiles.filter(
        (file) =>
          ...
          /\.(png|jpg|jpeg|webp|gif|avif|ktx2|basis)$/i.test(file.name),
      ),
    [pendingFiles],
  );
  ...
  const isPendingBundleComplete =
    completeness.hasSkeleton &&
    completeness.hasAtlas &&
  ...
  const handleAssetPick = async (assetId: string) => {
    ...
    assetPicker?.setSelectedAssetId(assetId);
    setIsOpen(false);
    try {
  ...
  const handleSubmitBundle = async () => {
    try {
      ...
      if (missingAtlasImages.length > 0) {
        throw new Error(
          t('toolRouteControls.errors.missingAtlasImages', { count: missingAtlasImages.length }),
        );
      }
    } catch (caughtError) {
      setUploadError(
        caughtError instanceof Error
          ? caughtError.message
          : t('toolRouteControls.errors.incompleteBundle'),
      );
      return;
    }
    ...
    try {
      ...
      setUploadError(
        caughtError instanceof Error
          ? caughtError.message
          : t('toolRouteControls.errors.uploadFailed'),
      );
    } finally {
      ...
      <header className="route-header-card">
        ...
        <div className="route-header-actions" aria-label={t('ui.routeUtilities')}>
          {assetPicker && (
            <button
              type="button"
              className="route-header-chip route-header-chip-asset"
              onClick={() => setIsOpen(true)}
            >
              <FolderOpenIcon className="route-header-chip-icon" size={13} />
              <span>
                {assetPicker.triggerLabel || t('toolRouteControls.actions.openPicker')}
              </span>
            </button>
          )}
          <button
      ...
      {assetPicker && isOpen && (
        <div className="tool-picker-modal-backdrop" onClick={() => setIsOpen(false)}>
          <section className="tool-picker-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div
              className="tool-modal-tabs"
              role="tablist"
              aria-label={t('toolRouteControls.tabs.assetOptions')}
            >
              <button
                type="button"
                role="tab"
            ...
            <div className="tool-picker-modal-body">
                    ...
                    {assets.map((asset) => (
                      ...
                      >
                        ...
                        <div className="tool-asset-card-copy">
                          <h3>{asset.name}</h3>
                          <p>
                            {t('toolRouteControls.values.assetFileCount', {
              ...
              {activeTab === 'import' && (
                <>
                  <div
                    className="tool-import-tabs"
                    role="tablist"
                    aria-label={t('toolRouteControls.tabs.importSource')}
                  >
                    <button
                      type="button"
                      role="tab"
                  ...
                  {onLoadFromUrl && importSource === 'url' && (
                    <>
                      ...
                      <div className="tool-url-form">
                        ...
                        <div className="form-group">
                          <label htmlFor="tool-image-urls">
                            {t('toolRouteControls.url.imageUrlsLabel')}
                          </label>
                          <textarea
                            id="tool-image-urls"
                        ...
                        <div className="tool-url-actions">
                          <button
                            ...
                            disabled={
                              isLoadingFromUrl || (!jsonUrl && !atlasUrl && !imageUrlsText)
                            }
                          >
                            {t('ui.cancel')}
                          </button>
                          ...
                          >
                            {isLoadingFromUrl
                              ? t('toolRouteControls.actions.loading')
                              : t('ui.load')}
                          </button>
                        </div>
                        {generatedSmartLink && (
                          <div className="tool-smart-link-preview">
                            <label htmlFor="tool-smart-link">
                              {t('toolRouteControls.url.smartLinkLabel')}
                            </label>
                            <input
                              id="tool-smart-link"
                              type="text"
                  ...
                  {importSource === 'file' && (
                    <>
                      ...
                      >
                        ...
                        <p className="spine-import-dropzone-title">
                          {t('toolRouteControls.dropzone.title')}
                        </p>
                        <p className="spine-import-dropzone-hint">
                          {t('toolRouteControls.dropzone.hint')}
                        </p>
                      </div>
                      ...
                      <div className="spine-file-slots">
                        <div className="spine-file-slot">
                          <div
                            className={`spine-file-slot-icon${pendingSkeletonFile ? ' filled' : ''}`}
                          >
                            {pendingSkeletonFile ? (
                              <svg
                                width="16"
                          ...
                          <div className="spine-file-slot-info">
                            <span className="spine-file-slot-label">
                              {t('toolRouteControls.fileSlots.skeleton')}
                            </span>
                            {pendingSkeletonFile && (
                              <span className="spine-file-slot-name">
                                {pendingSkeletonFile.name}
                              </span>
                            )}
                          </div>
                          <div className="spine-file-slot-actions">
                            {pendingSkeletonFile ? (
                              <button
                                ...
                                onClick={() => removeFile(pendingSkeletonFile.name)}
                              >
                        ...
                        <div className="spine-file-slot">
                          <div
                            className={`spine-file-slot-icon${pendingAtlasFile ? ' filled' : ''}`}
                          >
                            {pendingAtlasFile ? (
                              <svg
                                width="16"
                          ...
                          <div className="spine-file-slot-info">
                            <span className="spine-file-slot-label">
                              {t('toolRouteControls.fileSlots.atlas')}
                            </span>
                            {pendingAtlasFile && (
                              <span className="spine-file-slot-name">
                                {pendingAtlasFile.name}
                              </span>
                            )}
                          </div>
                          <div className="spine-file-slot-actions">
                            {pendingAtlasFile ? (
                              <button
                                type="button"
                                className="mini-btn danger"
                                onClick={() => removeFile(pendingAtlasFile.name)}
                              >
                                {t('toolRouteControls.fileSlots.remove')}
                              </button>
                            ) : (
                        ...
                        <div className="spine-file-slot spine-file-slot-textures">
                          <div
                            className={`spine-file-slot-icon${pendingTextureFiles.length > 0 && missingAtlasImages.length === 0 ? ' filled' : ''}`}
                          >
                          ...
                          <div className="spine-file-slot-info">
                            <span className="spine-file-slot-label">
                              {t('toolRouteControls.fileSlots.textures')}
                            </span>
                          </div>
                          <div className="spine-file-slot-actions">
                            <button
                        ...
                        {requiredAtlasImages.length > 0 && (
                          <div className="spine-texture-badges-section">
                            <span className="spine-texture-badges-label">
                              {t('toolRouteControls.fileSlots.requiredByAtlas')}
                            </span>
                          ...
                          <div className="spine-texture-files">
                            {pendingTextureFiles.map((file) => (
                              <div key={file.name} className="spine-texture-file">
                                ...
                                <button
                                  type="button"
                                  className="spine-texture-file-remove"
                      ...
                      <div className="tool-modal-footer">
                        ...
                        >
                          {isUploading
                            ? t('toolRouteControls.actions.importing')
                            : t('toolRouteControls.actions.import')}
                        </button>
                      </div>
                    </>
            ...
            <input
              ...
              multiple
              className="hidden-input"
              accept="image/*,.png,.jpg,.jpeg,.webp,.gif,.avif,.ktx2,.basis"
              onChange={handleInputFiles}
            />
          </section>
        </div>
